...
import com.paywith.ibeacon.service.IBeaconService;
import com.paywith.ibeacon.service.RegionData;
import com.paywith.ibeacon.service.StartRMData;
...
public class IBeaconManager {
     ...
     */
    public static final long DEFAULT_BACKGROUND_BETWEEN_SCAN_PERIOD = 10*1000; // 10 seconds
    ...
    public static double alertRangeNear = DEFAULT_ALERT_RANGE_NEAR;
    ...
    public void getLock() {
        ...
        if (serviceMessenger == null) {
            Log.w(TAG, "The IBeaconManager is not bound to the service.  Call iBeaconManager.bind(IBeaconConsumer consumer) and wait for a callback to onIBeaconServiceConnect()");
            return;
        }

        Message msg = Message.obtain(null, IBeaconService.MSG_SET_DO_LOCK, 0, 0);
        Log.d(TAG, "locking ");
        try {
            serviceMessenger.send(msg);
        }
    ...
    public void releaseLock(){
        ...
        Message msg = Message.obtain(null, IBeaconService.MSG_SET_DO_UNLOCK, 0, 0);
        Log.d(TAG, "unlocking ");
        try {
            serviceMessenger.send(msg);
        }
        catch(Exception e){
            Log.e(TAG, "unlocking ",e);
        }
    }
}
